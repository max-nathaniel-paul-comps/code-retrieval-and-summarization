...
class Seqifier(object):
    ...
    def __init__(self, seq_type, path, target_vocab_size=None, training_texts=None):
        ...
        if seq_type == 'regex_based' or seq_type == 'antlr_csharp':
            ...
            self.seqify_texts = lambda texts: [subword_encoder.encode(tdu.eof_text(text)) for text in texts]
            self.de_seqify_texts = lambda seqs: [subword_encoder.decode(tdu.eof_text(seq)) for seq in seqs]
